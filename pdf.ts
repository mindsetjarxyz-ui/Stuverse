...
export function Verification() {
  ...
  const { user, refreshUser, logout, resendVerification } = useAuth();
      ...
      >
        ...
        <div className="space-y-4">
          <button 
            ...
            disabled={checking}
            className="w-full py-4 glass-pill bg-indigo-500 text-white font-semibold flex items-center justify-center gap-3 hover:bg-indigo-600 transition-all shadow-[0_0_20px_rgba(99,102,241,0.2)] disabled:opacity-50"
          >
            {checking ? (
          ...
          <button 
            ...
            className={`w-full py-4 glass-pill border border-white/10 text-white font-semibold flex items-center justify-center gap-3 transition-all disabled:opacity-50 ${resendSuccess ? 'bg-emerald-500/20 border-emerald-500/30' : 'hover:bg-white/5'}`}
          >
            {resending ? (
              <RefreshCw className="w-5 h-5 animate-spin" />
            ) : resendSuccess ? (
          ...
          <button 
            ...
            className="w-full py-4 text-gray-500 font-medium flex items-center justify-center gap-2 hover:text-white transition-colors"
          >
            Back to Login
          </button>
        </div>
